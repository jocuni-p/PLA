...
<div class='row animated fadeIn slow'>
    <div class='column col-8'>
        <div class="card m-auto">
            @if (isset($pelicula))
            <form id='formulario' action="{{ route('crud.pelicula.modificacion', [$pelicula->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <h2 class="card-title">
                        <input name='titulo' type='text' value="{{ $pelicula->titulo ?? '' }}" class="form-control">
                    </h2>
                    <hr>
                    <h5 class="card-subtitle mb-2 text-muted">Dirección:
                        <input name='direccion' value="{{ $pelicula->direccion ?? '' }}" class="form-control">
                    </h5>
                    <h5 class="card-subtitle mb-2 text-muted">Año:
                        <input name='anio' type='number' min='1900' max='2100' value="{{ $pelicula->anio ?? '' }}" class="form-control">
                    </h5>
                    <hr>
                    <textarea rows='8' cols='90' name='sinopsis' class="form-control">{{ $pelicula->sinopsis ?? '' }}</textarea>
                    <hr>
